...
## Несколько хостов — один вызов вместо десяти

Если передать больше одного хоста, примеры используют **пакетный эндпоинт** `/api/Certificate/batch`. До **10 хостов проверяются за один вызов**: одно HTTP-соединение вместо десяти, а сервер снимает сертификаты параллельно — пачка отвечает почти за то же время, что и одиночная проверка.

Экономия здесь **сетевая, а не тарифная**: каждый хост — это отдельное TLS-рукопожатие, поэтому пакет из N хостов тарифицируется как N одиночных проверок и на столько же расходует rate-limit. Повторы одного и того же хоста в списке схлопываются (без учёта регистра) — за них платить дважды не придётся.
...
Один прогон примера — **ровно один HTTP-запрос к API**, сколько бы хостов вы ни передали (до 10). Тарифицируется он по числу проверенных хостов.
...
|----------|-----|-----|----------|
| `host` | query | string | Доменное имя или публичный IP: `example.com`, `www.example.com`, `93.184.216.34`. **Только публичные адреса.** Хост, который резолвится во внутреннюю сеть (`192.168.*`, `10.*`, `localhost`), проверен не будет: придёт `200` с `isValid: false` и `errors: ["Target host is not allowed."]` |
...
| Параметр | Где | Тип | Описание |
|----------|-----|-----|----------|
| `hosts` | query | string | Хосты через запятую: `a.com,b.com,c.com`. **Максимум 10** за запрос, пробелы вокруг запятых допустимы. Повторы схлопываются без учёта регистра: `a.com,A.com` — это один хост, и считается он за один |
...
Пакетный эндпоинт возвращает **массив** таких объектов — в том же порядке, в каком перечислены хосты. Обратите внимание: повторы из списка выброшены, поэтому элементов в массиве может оказаться меньше, чем вы передали. Сопоставляйте результат с запросом **по полю `host`, а не по позиции.**

Поле `checkedAt` — это момент, когда проверка была фактически выполнена. Он может оказаться раньше момента вашего запроса: сервис не обязан открывать новое TLS-соединение на каждый вызов. Признака «ответ отдан из хранилища» в контракте нет и на цену это не влияет — свежесть данных видно именно по `checkedAt`.
...
| `500` | Непредвиденный сбой на нашей стороне | Повторить позже. **Резерв снимается целиком — за сорванный запрос деньги не списываются** |

**Недоступный хост — это НЕ ошибка HTTP.** Таймаут, отказ в соединении, отсутствие TLS на порту, неизвестный домен — всё это приходит как `200 OK` с `isValid: false` и причиной в `errors[]`, а не как `500`. Так и задумано: проверка выполнена, просто её результат отрицательный, и для мониторинга парка сертификатов это ровно тот ответ, за которым вы пришли. Клиент, который смотрит только на HTTP-код, примет упавший хост за исправный — примеры проверяют именно `isValid` и `errors`.
...
Оплата **pay-as-you-go, без подписки**: платите только за выполненные проверки. Пакетный эндпоинт тарифицируется **похостно**: пачка из N хостов стоит столько же, сколько N одиночных проверок, и на столько же расходует rate-limit. Пачка экономит сетевой обход и время, а не деньги. Если весь запрос завершился ошибкой, не списывается ничего.
...
- [Аудит сайта](https://github.com/atlorium-api/core-web-vitals-api-client) — Core Web Vitals и скорость загрузки страницы
...
- [Проверка ссылки](https://github.com/atlorium-api/url-reputation-api-client) — репутация URL: фишинг, вредонос, нежелательное ПО
